...
`+JSON.stringify(o,null,2):m=" "+this.formatData(o));let u="";if(n){let{sessionId:E,memorySessionId:g,correlationId:h,...l}=n;Object.keys(l).length>0&&(u=` {${Object.entries(l).map(([f,O])=>`${f}=${O}`).join(", ")}}`)}let T=`[${i}] [${a}] [${_}] ${p}${s}${u}${m}`;if(this.logFilePath)try{(0,A.appendFileSync)(this.logFilePath,T+`
...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),m=p?`${p[1].split("/").pop()}:${p[2]}`:"unknown",u={...s,location:m};return this.warn(e,`[HAPPY-PATH] ${t}`,u,n),o}},c=new W;var Nt={};function Ot(){return typeof __dirname<"u"?__dirname:(0,b.dirname)((0,pe.fileURLToPath)(Nt.url))}var Rt=Ot(),R=C.get("AI_MEM_DATA_DIR"),y=process.env.CLAUDE_CONFIG_DIR||(0,b.join)((0,de.homedir)(),".claude"),Vt=(0,b.join)(y,"plugins","marketplaces","bdmorin"),qt=(0,b.join)(R,"archives"),Kt=(0,b.join)(R,"logs"),Jt=(0,b.join)(R,"trash"),Qt=(0,b.join)(R,"backups"),zt=(0,b.join)(R,"modes"),Zt=(0,b.join)(R,"settings.json"),ce=(0,b.join)(R,"ai-mem.db"),es=(0,b.join)(R,"vector-db"),ts=(0,b.join)(R,"observer-sessions"),ss=(0,b.join)(y,"settings.json"),rs=(0,b.join)(y,"commands"),ns=(0,b.join)(y,"CLAUDE.md");function me(r){(0,_e.mkdirSync)(r,{recursive:!0})}function ue(){return(0,b.join)(Rt,"..")}var le=require("crypto");var At=3e4;function Ee(r,e,t){return(0,le.createHash)("sha256").update((r||"")+(e||"")+(t||"")).digest("hex").slice(0,16)}function Te(r,e,t){let s=t-At;return r.prepare("SELECT id, created_at_epoch FROM observations WHERE content_hash = ? AND created_at_epoch > ?").get(e,s)}var U=class{db;constructor(e=ce){e!==":memory:"&&me(R),this.db=new ge.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys(),this.addObservationContentHashColumn(),this.addSessionCustomTitleColumn()}initializeSchema(){this.db.run(`
