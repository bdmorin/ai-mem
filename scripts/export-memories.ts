 ...
 * Example: npx tsx scripts/export-memories.ts "windows" windows-memories.json --project=ai-mem
 */
...
async function exportMemories(query: string, outputFile: string, project?: string) {
  try {
    ...
    const settings = SettingsDefaultsManager.loadFromFile(join(homedir(), '.claude', 'ai-mem-data', 'settings.json'));
...
if (args.length < 2) {
  ...
  console.error('Example: npx tsx scripts/export-memories.ts "windows" windows-memories.json --project=ai-mem');
