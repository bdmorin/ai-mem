 ...
 * - CLI: `ai-mem generate` / `ai-mem clean`
...
const DB_PATH = path.join(os.homedir(), '.claude', 'ai-mem-data', 'ai-mem.db');
const SETTINGS_PATH = path.join(os.homedir(), '.claude', 'ai-mem-data', 'settings.json');
...
function walkDirectoriesWithIgnore(dir: string, folders: Set<string>, depth: number = 0): void {
  ...
  const ignorePatterns = [
    ...
    '.claude', 'ai-mem-data', '.open-next', '.turbo'
  ];
...
function formatObservationsForClaudeMd(observations: ObservationRow[], folderPath: string): string {
  ...
  lines.push('');
  lines.push('<!-- This section is auto-generated by ai-mem. Edit content outside the tags. -->');
  lines.push('');
  ...
  try {
    ...
    for (const file of filesToProcess) {
      ...
      try {
        ...
        const stripped = content.replace(/<ai-mem-context>[\s\S]*?<\/ai-mem-context>/g, '').trim();
